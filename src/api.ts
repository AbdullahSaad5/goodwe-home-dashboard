...
export const api = {
  ...
  history: (period: Period, anchor?: string) => getJson<HistoryResponse>(`/api/v1/history?period=${period}${anchor ? `&anchor=${anchor}` : ''}`),
  summary: (period: Period, anchor?: string) => getJson<Summary>(`/api/v1/summary?period=${period}${anchor ? `&anchor=${anchor}` : ''}`),
