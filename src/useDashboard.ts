...
export function useDashboard(period: Period, anchor: string) {
  ...
  const noteSnapshotArrival = useCallback((value: Snapshot) => {
    ...
    const nextInterval = Math.min(liveRefreshEstimate.current * retryMultiplier, MAX_LIVE_REFRESH_MS);
  ...
  const refreshHistory = useCallback(async () => {
    ...
    try {
      const [newHistory, newSummary] = await Promise.all([
        api.history(period, anchor), api.summary(period, anchor),
      ]);
      if (request !== historyRequest.current) return;
      setHistory(newHistory); setSummary(newSummary);
      ...
      if (request === historyRequest.current && import.meta.env.DEV) {
        setHistory({ ...demoHistory, period }); setSummary({ ...demoSummary, period }); setComparisonSummary(null);
        setPreview(true);
      }
    }
  }, [anchor, period]);
  ...
  const refreshReferenceData = useCallback(async () => {
    try {
      ...
      setSensors(newSensors); setEvents(newEvents);
    } catch {
      if (import.meta.env.DEV) {
        setSensors(demoSensors); setEvents(demoEvents); setPreview(true);
      }
    }
  }, []);
  ...
  useEffect(() => {
    let cancelled = false;
    api.status().then((value) => { if (!cancelled) { setSnapshot(value); noteSnapshotArrival(value); setPreview(false); } })
      .catch(() => { if (!cancelled && import.meta.env.DEV) { setSnapshot(demoSnapshot); noteSnapshotArrival(demoSnapshot); setPreview(true); } })
  ...
  useEffect(() => {
    ...
    stream.addEventListener('snapshot', (event) => {
      try {
        ...
        setSnapshot(value); noteSnapshotArrival(value); setPreview(false); setLoading(false);
      }
      catch { /* Ignore malformed updates; the next poll will recover. */ }
    });
    return () => stream.close();
  }, [noteSnapshotArrival]);

  return { snapshot, history, summary, comparisonSummary, sensors, events, preview, loading, nextRefreshAt, refreshHistory, refreshReferenceData };
}
