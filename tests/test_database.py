...
from backend.normalization import normalize_snapshot
...
def test_rollup_preserves_aggregates_before_retention(tmp_path) -> None:
    ...
    with database._lock:
        ...
        aggregate_count = database._connection.execute("SELECT COUNT(*) FROM aggregates").fetchone()[0]
...
def test_anchor_uses_reporting_calendar_day(tmp_path) -> None:
    ...
    database.record_snapshot(normalize_snapshot(raw_sample(ppv=1100), collected_at=before_midnight), raw_sample())
    database.record_snapshot(normalize_snapshot(raw_sample(ppv=3300), collected_at=next_day_boundary), raw_sample())
    database.record_snapshot(normalize_snapshot(raw_sample(ppv=2200), collected_at=after_midnight), raw_sample())
