  ...
  if (!visible.length)
    return (
      <EmptyState
        ...
        detail="Collector and inverter events will appear here."
      />
    );
  return (
...
function Comparison({
  ...
  return (
    <span className={`comparison ${desirable ? 'positive' : 'negative'}`}>
      {positive ? '▲' : '▼'} {formatNumber(Math.abs(value), 0)}% vs {reference}
    </span>
  );
}
...
function DailyCard({
  ...
  return (
    <Card className={`daily-card ${tone}`}>
      ...
      <dl className="daily-details">
        <Detail label={leftLabel} value={leftValue} />
        <Detail label={rightLabel} value={rightValue} />
      </dl>
    </Card>
  );
}
...
function MpptPanel({ snapshot }: { snapshot: Snapshot }) {
  return (
    <Panel title="MPPT Details" className="operational-card mppt-card">
      <div className="mppt-list">
        {[snapshot.solar.mppt1, snapshot.solar.mppt2].map((mppt, index) => (
          <div className="mppt-item" key={index}>
            <strong>MPPT {index + 1}</strong>
            <Detail label="Voltage" value={`${formatNumber(mppt.voltage_v)} V`} />
            <Detail label="Current" value={`${formatNumber(mppt.current_a)} A`} />
            <Detail label="Power" value={formatPower(mppt.power_w)} />
          </div>
        ))}
      </div>
      <div className="card-summary">
        ...
        <strong>
          <AnimatedReading value={formatPower(snapshot.solar.total_power_w)} />
        </strong>
      </div>
    </Panel>
  );
...
function BatteryStatusPanel({ snapshot }: { snapshot: Snapshot }) {
  ...
  return (
    <Panel
      title={
        <span className="operational-title">
          <span className="title-icon battery">
            <BatteryCharging />
          </span>
          Battery Status
        </span>
      }
      className="operational-card battery-status-card"
    >
      <dl className="status-list">
        <Detail label="Temperature" value={`${formatNumber(battery.temperature_c)} °C`} />
        <Detail label="Health" value={snapshot.insights.cell_balance} />
        <Detail
          label="Cell spread"
          value={`${formatNumber(battery.cell_voltage_spread_mv, 0)} mV`}
        />
        <Detail label="Voltage" value={`${formatNumber(battery.voltage_v)} V`} />
        <Detail label="Current" value={`${formatNumber(battery.current_a)} A`} />
      </dl>
      <p className="panel-note">
        <CircleCheck />
        {battery.warning || battery.error || 'No issues detected'}
      </p>
    </Panel>
  );
}
...
function GridQualityPanel({ snapshot }: { snapshot: Snapshot }) {
  ...
  return (
    <Panel
      title={
        <span className="operational-title">
          <span className="title-icon grid">
            <Gauge />
          </span>
          Grid Quality
        </span>
    ...
    >
      <dl className="status-list">
        <Detail label="Voltage" value={`${formatNumber(grid.voltage_v)} V`} />
        <Detail label="Frequency" value={`${formatNumber(grid.frequency_hz, 2)} Hz`} />
        <Detail label="Reactive power" value={`${formatNumber(grid.reactive_var, 0)} var`} />
        <Detail label="Apparent power" value={`${formatNumber(grid.apparent_va, 0)} VA`} />
        <Detail
          label="Meter link"
          value={grid.meter_communicating ? 'Communicating' : 'Unavailable'}
        />
      </dl>
      <p className="panel-note">
        <CircleCheck />
        {grid.meter_communicating
          ? 'Grid conditions are being monitored'
          : 'Meter data is unavailable'}
      </p>
    </Panel>
  );
}
  ...
  return (
    <>
      ...
      <div className="daily-grid">
        ...
        <DailyCard
          title="Battery SOC"
          value={formatNumber(snapshot.battery.soc_pct, 0)}
          unit="%"
          ...
          rightLabel="Power"
          rightValue={formatPower(snapshot.power.battery_w, true)}
        />
      </div>
      ...
      <div className="operations-grid">
        ...
        <Panel
          title={
            <span className="operational-title">
              <span className="title-icon notification">
                <Bell />
              </span>
              Alerts & Notifications
            </span>
          }
          className="operational-card event-card"
        >
          <EventList events={events} limit={3} />
          <button type="button" className="event-footer" onClick={onViewAlerts}>
            View all alerts
            <ChevronRight />
          </button>
        </Panel>
  ...
  return (
    <>
      <PageIntro
        ...
        description="Synchronized solar, load, battery and grid data across calendar periods."
        hero={
          <HistoryControls
            period={period}
            setPeriod={setPeriod}
      ...
      />
      <Panel
        title="Generation, demand and exchange"
        eyebrow={periodLabel(period)}
        className="feature-panel"
        action={
          <a
            className={buttonVariants({ variant: 'outline', size: 'sm' })}
            href={`/api/v1/export.csv?period=${period}&anchor=${anchor}`}
          >
            <Download />
            CSV
          </a>
      ...
      <div className="stat-grid">
        <StatCard
          label="Generated"
          value={formatNumber(summary?.energy.solar_kwh)}
          unit="kWh"
          tone="solar"
        />
        <StatCard
          label="Consumed"
          value={formatNumber(summary?.energy.load_kwh)}
          unit="kWh"
          tone="home"
        />
        <StatCard
          label="Imported"
          value={formatNumber(summary?.energy.import_kwh)}
          unit="kWh"
          tone="grid"
        />
        <StatCard
          label="Exported"
          value={formatNumber(summary?.energy.export_kwh)}
          unit="kWh"
          tone="battery"
        />
      </div>
      <div className="stat-grid">
        <StatCard label="Peak solar" value={formatPower(summary?.peak_pv_w)} tone="solar" />
        <StatCard label="Peak home" value={formatPower(summary?.peak_home_w)} tone="home" />
        <StatCard
          label="Grid independence"
          value={formatNumber(summary?.grid_independence_pct)}
          unit="%"
          tone="battery"
        />
        <StatCard label="Availability" value={formatNumber(summary?.availability_pct)} unit="%" />
      </div>
    </>
  );
}
    ...
    <>
      ...
      <div className="stat-grid">
        <StatCard
          ...
          value={formatNumber(snapshot.solar.today_kwh)}
          unit="kWh"
          tone="solar"
        />
        <StatCard
          label="Lifetime yield"
          value={formatNumber(snapshot.solar.lifetime_kwh)}
          unit="kWh"
          tone="solar"
        />
        <StatCard
          label="Operating time"
          value={formatNumber(snapshot.solar.operating_hours, 0)}
          unit="hours"
        />
        <StatCard
          label="Current output"
          value={formatPower(snapshot.solar.total_power_w)}
          tone="solar"
        />
      </div>
      ...
      <div className="two-column">
        {[snapshot.solar.mppt1, snapshot.solar.mppt2].map((mppt, index) => (
          <Panel key={index} title={`MPPT ${index + 1}`} eyebrow={mppt.mode}>
    ...
    <>
      ...
      <div className="stat-grid">
        <StatCard
          ...
          value={formatNumber(battery.soc_pct, 0)}
          unit="%"
          tone="battery"
        />
        <StatCard
          label="State of health"
          value={formatNumber(battery.soh_pct, 0)}
          unit="%"
          tone="battery"
        />
        <StatCard
          label="Battery power"
          value={formatPower(battery.power_w, true)}
          note={snapshot.power.battery_direction}
        />
      ...
      <div className="two-column">
        <Panel title="Electrical">
          <dl className="details-grid">
            <Detail label="Voltage" value={`${formatNumber(battery.voltage_v)} V`} />
            <Detail label="Current" value={`${formatNumber(battery.current_a)} A`} />
            <Detail label="Charge limit" value={`${formatNumber(battery.charge_limit_a)} A`} />
            <Detail
              label="Discharge limit"
              value={`${formatNumber(battery.discharge_limit_a)} A`}
            />
            <Detail label="Modules" value={formatNumber(battery.modules, 0)} />
          </dl>
        </Panel>
        <Panel title="Cells & BMS">
          <dl className="details-grid">
            <Detail
              label="Cell voltage range"
              value={`${formatNumber(battery.min_cell_voltage_v, 3)}–${formatNumber(battery.max_cell_voltage_v, 3)} V`}
            />
            <Detail
              label="Voltage spread"
              value={`${formatNumber(battery.cell_voltage_spread_mv, 0)} mV`}
              note={snapshot.insights.cell_balance}
            />
            <Detail
              label="Cell temperature"
              value={`${formatNumber(battery.min_cell_temperature_c)}–${formatNumber(battery.max_cell_temperature_c)} °C`}
            />
            <Detail label="Software" value={battery.software_version || '—'} />
            <Detail label="Hardware" value={battery.hardware_version || '—'} />
          </dl>
        </Panel>
      </div>
    </>
...
export function GridPage({
  ...
  return (
    <>
      ...
      />
      <div className="stat-grid">
        <StatCard label="Home load" value={formatPower(snapshot.power.home_w)} tone="home" />
        <StatCard label="Backup load" value={formatPower(snapshot.power.backup_w)} />
        <StatCard
          label="Grid exchange"
          value={formatPower(snapshot.power.grid_w, true)}
          note={snapshot.power.grid_direction}
          tone="grid"
        />
        <StatCard label="Inverter output" value={formatPower(grid.inverter_output_w)} />
      </div>
      <Panel
        title="Load and grid exchange"
      ...
      <div className="two-column">
        <Panel title="Mains quality">
          <dl className="details-grid">
            <Detail label="Voltage" value={`${formatNumber(grid.voltage_v)} V`} />
            <Detail label="Current" value={`${formatNumber(grid.current_a)} A`} />
            <Detail label="Frequency" value={`${formatNumber(grid.frequency_hz, 2)} Hz`} />
            <Detail
              label="Meter link"
              value={grid.meter_communicating ? 'Communicating' : 'Unavailable'}
            />
            <Detail label="Grid state" value={grid.grid_mode} />
            <Detail label="Exchange mode" value={grid.on_grid_mode} />
          </dl>
        </Panel>
        <Panel title="Power and energy">
          <dl className="details-grid">
            <Detail label="Active power" value={`${formatNumber(grid.meter_active_w, 0)} W`} />
            <Detail label="Reactive power" value={`${formatNumber(grid.reactive_var, 0)} var`} />
            <Detail label="Apparent power" value={`${formatNumber(grid.apparent_va, 0)} VA`} />
            <Detail
              label="Imported today"
              value={`${formatNumber(snapshot.today.import_kwh)} kWh`}
            />
            <Detail
              label="Exported today"
              value={`${formatNumber(snapshot.today.export_kwh)} kWh`}
            />
            <Detail
              label="Net export"
              value={`${formatNumber(snapshot.insights.net_grid_kwh)} kWh`}
            />
          </dl>
        </Panel>
      </div>
  ...
  return (
    <>
      <PageIntro
        eyebrow="Diagnostics"
        title="System status"
        description="Read-only inverter health, temperatures, clocks, firmware and local collector events."
        hero={<HealthBadge health={system.health} />}
      />
      <div className="stat-grid">
        <StatCard
          label="Air temperature"
          value={formatNumber(system.temperature_air_c)}
          unit="°C"
        />
        <StatCard
          label="Module temperature"
          value={formatNumber(system.temperature_module_c)}
          unit="°C"
        />
        <StatCard
          label="Radiator temperature"
          value={formatNumber(system.temperature_radiator_c)}
          unit="°C"
        />
        <StatCard
          label="Clock drift"
          value={formatNumber(snapshot.connection.clock_drift_seconds, 0)}
          unit="seconds"
        />
      </div>
      <div className="system-feature-grid">
        <Panel title="Inverter" action={<ShieldCheck />}>
          <div className="system-overview-body">
            <img src="/solar-home.png" alt="" className="solar-home-art" />
            <dl className="details-grid">
              <Detail label="Model" value={snapshot.connection.display_model} />
              <Detail label="Protocol" value={snapshot.connection.protocol_model || '—'} />
              <Detail label="Firmware" value={snapshot.connection.firmware || '—'} />
              <Detail label="Work mode" value={system.work_mode} />
            </dl>
          </div>
        </Panel>
      ...
      <div className="two-column">
        <Panel title="System details" action={<Server />}>
          <dl className="details-grid">
            <Detail label="Operation code" value={formatNumber(system.operation_mode_code, 0)} />
            <Detail
              label="Safety profile"
              value={system.safety_country || String(system.safety_country_code)}
            />
            <Detail label="Warning code" value={formatNumber(system.warning_code, 0)} />
            <Detail label="Error code" value={formatNumber(system.error_code, 0)} />
          </dl>
        </Panel>
        <Panel title="Status registers" action={<Gauge />}>
          <dl className="details-grid">
            <Detail label="Errors" value={system.errors || 'None'} />
            <Detail
              label="Diagnostic flags"
              value={system.diagnostic || 'None'}
              note="Informational unless fault registers are set"
            />
          </dl>
        </Panel>
      </div>
      <Panel title="Event history" eyebrow={`${events.length} local events`}>
...
export function RawPage({ sensors }: { sensors: SensorReading[] }) {
  ...
  const categories = useMemo(
    () => ['ALL', ...Array.from(new Set(sensors.map((sensor) => sensor.category))).sort()],
    [sensors],
  );
  const rows = useMemo(
    () =>
      sensors.filter(
        (sensor) =>
          ...
          `${sensor.id} ${sensor.name} ${sensor.value}`.toLowerCase().includes(query.toLowerCase()),
      ),
    [sensors, query, category],
  );
  return (
    <>
      <PageIntro
        ...
        description="Every original inverter reading, including values hidden from normal single-phase views."
        hero={
          <div className="hero-reading neutral">
            <Database />
            {sensors.length}
      ...
      <Panel
        ...
        action={
          <div className="raw-tools">
            <SearchField value={query} onChange={setQuery} placeholder="Search name, ID or value" />
            <select
      ...
      >
        <div className="table-wrap">
            ...
            <tbody>
              {rows.map((sensor) => (
                <tr key={sensor.id}>
                  ...
                  <td>{sensor.name}</td>
                  <td className="value-cell">{String(sensor.value ?? '—')}</td>
                  <td>{sensor.unit}</td>
                  <td>
          ...
          {!rows.length && <EmptyState compact title="No matching sensors" />}
        </div>
      </Panel>
    </>
  );
